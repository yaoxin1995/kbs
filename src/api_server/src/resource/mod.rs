...
pub(crate) async fn get_secret_resource(
    ...
    let resource_byte = if resource_desc.resource_type == "signing_key" {

        info!("resource type is  signing_key");
        ...
        info!("generate private_key {}", &*private_key_pem);

        (&*private_key_to_byte).clone()
    } else {
        repository
        .read()
        .await
    ...
    let s = String::from_utf8_lossy(&resource_byte);

    info!("get_secret_resource resource_byte {:?}, resource {:?}", resource_byte.len(), s);
